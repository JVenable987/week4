...
router.delete('/movies', passport.authenticate("basic", {session:false}), function(req, res){
    console.log(req.body);
        res = res.status(200);
        res.json({
            headers: req.headers,
...
router.all('/*', function(req, res){
    res = res.status(404).res = res.status(404).send({success: false, msg: "page not found"})
});
